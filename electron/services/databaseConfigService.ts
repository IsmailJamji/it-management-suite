...
const SYNC_FOLDER_NAME = 'IT-Management-Cloud';
const DB_FILE_NAME = 'it_management.db';
...
export class DatabaseConfigService {
  ...
  public async setMode(mode: 'server' | 'client' | 'local' | 'cloud', serverInfo?: { name: string; ip: string }): Promise<void> {
    ...
    switch (mode) {
      case 'server':
        ...
        await this.ensureDirectoryExists(sharedDataPath);
        console.log('✅ Shared directory created/verified');
        
        dbPath = path.join(sharedDataPath, DB_FILE_NAME);
      ...
      case 'cloud': {
        const cloudRoot = this.resolveOneDriveRoot();
        const cloudPath = cloudRoot
          ? path.join(cloudRoot, SYNC_FOLDER_NAME)
          : path.join(app.getPath('documents'), SYNC_FOLDER_NAME);
        await this.ensureDirectoryExists(cloudPath);
        dbPath = path.join(cloudPath, DB_FILE_NAME);
        isOnline = await this.checkConnection(dbPath);
        console.log('☁️ Cloud mode database path:', dbPath);
        if (!cloudRoot) {
          console.warn('⚠️ OneDrive not detected — using Documents folder as fallback');
        }
        break;
      }
  ...
  /** Détecte le dossier racine OneDrive (personnel, entreprise, variable d'environnement). */
  public resolveOneDriveRoot(): string | null {
    ...
    try {
      ...
      for (const entry of fs.readdirSync(home, { withFileTypes: true })) {
        ...
        if (name.toLowerCase().startsWith('onedrive')) {
          ...
          if (fs.existsSync(full) && !candidates.includes(full)) {
            candidates.push(full);
          }
        }
      }
    } catch (error) {
      console.warn('Could not scan home directory for OneDrive:', error);
    }

    return candidates[0] ?? null;
  }

  public resolveGoogleDriveRoot(): string | null {
    const candidates: string[] = [];
    const envPath = process.env['Google Drive'] || process.env.GOOGLE_DRIVE;
    if (envPath && fs.existsSync(envPath)) {
      candidates.push(path.normalize(envPath));
    }

    const defaultPath = path.join(os.homedir(), 'Google Drive');
    if (fs.existsSync(defaultPath) && !candidates.includes(defaultPath)) {
      candidates.push(defaultPath);
    }

    try {
      for (const entry of fs.readdirSync(os.homedir(), { withFileTypes: true })) {
        if (entry.isDirectory() && entry.name.toLowerCase().includes('google drive')) {
          const full = path.join(os.homedir(), entry.name);
          if (fs.existsSync(full) && !candidates.includes(full)) {
            candidates.push(full);
          }
        }
  ...
  public getOneDriveSyncFolderPath(): string | null {
    const root = this.resolveOneDriveRoot();
    return root ? path.join(root, SYNC_FOLDER_NAME) : null;
  }

  public getOneDriveDatabasePath(): string | null {
    const folder = this.getOneDriveSyncFolderPath();
    if (!folder || !fs.existsSync(folder)) {
      return null;
    }
    const dbPath = path.join(folder, DB_FILE_NAME);
    return fs.existsSync(dbPath) ? dbPath : null;
  }

  public getPotentialOneDriveDatabasePath(): string {
    const root = this.resolveOneDriveRoot();
    const base = root ?? path.join(os.homedir(), 'OneDrive');
    return path.join(base, SYNC_FOLDER_NAME, DB_FILE_NAME);
  }

  public getGoogleDriveDatabasePath(): string | null {
    const root = this.resolveGoogleDriveRoot();
    if (!root) return null;
    const folder = path.join(root, SYNC_FOLDER_NAME);
  ...
  public getPotentialGoogleDriveDatabasePath(): string {
    const root = this.resolveGoogleDriveRoot() ?? path.join(os.homedir(), 'Google Drive');
    return path.join(root, SYNC_FOLDER_NAME, DB_FILE_NAME);
  }

  private checkPathWithTimeout(filePath: string, timeoutMs = 1500): Promise<boolean> {
  ...
  public async getNetworkDatabasePaths(): Promise<string[]> {
    const commonPaths = [
    ...
    const results = await Promise.all(
      commonPaths.map(async (networkPath) => {
        const accessible = await this.checkPathWithTimeout(networkPath, 1500);
        return accessible ? networkPath : null;
      })
    );
  ...
  public async testDatabaseConnection(dbPath: string): Promise<boolean> {
    try {
      ...
      const probe = path.join(parentDir, `.it-suite-write-test-${Date.now()}`);
  ...
  public async createSyncFolder(type: 'onedrive' | 'googledrive'): Promise<SyncFolderResult> {
    try {
      let root: string | null;

      if (type === 'onedrive') {
        root = this.resolveOneDriveRoot();
        if (!root) {
          return {
            success: false,
            message:
              'Dossier OneDrive introuvable. Vérifiez que OneDrive est installé, connecté et synchronisé.',
          };
        }
      } else {
        root = this.resolveGoogleDriveRoot();
        if (!root) {
          return {
            success: false,
            message: 'Dossier Google Drive introuvable sur ce PC.',
          };
        }
      }

      const folderPath = path.join(root, SYNC_FOLDER_NAME);
      await this.ensureDirectoryExists(folderPath);

      const dbPath = path.join(folderPath, DB_FILE_NAME);

      const placeholderPath = path.join(folderPath, '.sync-folder-created');
      await fs.promises.writeFile(
        placeholderPath,
        `IT Suite sync folder\nCreated: ${new Date().toISOString()}\n`,
        'utf8'
      );

      console.log('✅ Sync folder ready:', folderPath);
      console.log('✅ Database path:', dbPath);

      return { success: true, dbPath, folderPath };
    } catch (error) {
  ...
  public async applyDatabasePathConfig(
    dbPath: string,
    mode?: 'server' | 'client' | 'local' | 'cloud'
  ): Promise<void> {
    const parentDir = path.dirname(dbPath);
    ...
    const resolvedMode = mode ?? this.config?.mode ?? 'local';
    ...
    this.config = {
      ...
      isOnline: true,
      lastSync: new Date(),
      serverInfo: resolvedMode === 'client' ? this.config?.serverInfo : undefined,
    };
    this.saveConfig();
  }
}
