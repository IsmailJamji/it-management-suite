...
import { isDev, getAppIconPath } from './utils';
...
function setupIPC(): void {
  ...
  ipcMain.handle('system-get-live-metrics', async () => {
    return await systemInfoCollector.getLiveMetrics();
  });

  ipcMain.handle('system-get-current-device-id', async () => {
...
ipcMain.handle('excel-import-execute', async (event, fileData, assetType) => {
  ...
  ipcMain.handle('installation-set-mode', async (event, mode, serverInfo) => {
    try {
      ...
      if (mode === 'server' || mode === 'local' || mode === 'cloud') {
        ...
        if (!database.isDatabaseInitialized(newDbPath)) {
          await database.initializeDatabase();
        }
      }
  ...
  ipcMain.handle('installation-set-database-path', async (event, newPath) => {
    try {
      const configService = database.getConfigService();
      const normalizedPath = newPath.replace(/database\.db$/i, 'it_management.db');

      const isCloud =
        normalizedPath.includes('OneDrive') ||
        normalizedPath.includes('Google Drive') ||
        normalizedPath.includes('IT-Management-Cloud');

      await configService.applyDatabasePathConfig(
        normalizedPath,
        isCloud ? 'cloud' : configService.getConfig()?.mode ?? 'local'
      );

      database.setDatabasePath(normalizedPath);

      if (!database.isDatabaseInitialized(normalizedPath)) {
        await database.initializeDatabase();
      }

      return { success: true, dbPath: normalizedPath };
    } catch (error) {
  ...
  ipcMain.handle('installation-get-network-database-paths', async () => {
    try {
      const configService = database.getConfigService();
      return await configService.getNetworkDatabasePaths();
    } catch (error) {
  ...
  ipcMain.handle('installation-create-sync-folder', async (event, type) => {
    try {
      const configService = database.getConfigService();
      const result = await configService.createSyncFolder(type);

      if (!result.success || !result.dbPath) {
        return result;
      }

      await configService.applyDatabasePathConfig(result.dbPath, 'cloud');
      database.setDatabasePath(result.dbPath);

      if (!database.isDatabaseInitialized(result.dbPath)) {
        await database.initializeDatabase();
      }
